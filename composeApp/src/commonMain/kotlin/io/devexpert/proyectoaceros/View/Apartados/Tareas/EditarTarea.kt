...
@Composable
fun EditarTarea (navController: Navigation, taskInfo: ReadTaskExisting, tareasViewModel: TareasViewModel) {
    ...
    Column (modifier = Modifier
        ...
        Column (modifier = Modifier.fillMaxWidth().height(200.dp)) {
            ...
            Text("ID: ${taskInfo.ActividadId}", modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
            Text(taskInfo.Descripcion, modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
            Text("Num Empleado: ${taskInfo.NumEmpleado}", modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
            Text("Area: ${taskInfo.Area}", modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
            Text("Margen de Error: ${taskInfo.MargenError}%", modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
            Text("Fecha de inicio: ${taskInfo.FechaInicio}", modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
            Text("Fecha de Fin: ${taskInfo.FechaFin}", modifier = Modifier.padding(top = 5.dp), style = MaterialTheme.typography.bodyLarge)
        }
            ...
            OutlinedButton(
                onClick = {
                    ...
                    if (!queryDescription.matches(notEmptyOrSpacesRegex)&&
                        ...
                        textAlert = "Aun sin campos para modificar"
                    } else {
                        textAlert = tareasViewModel.updateActivity(
                            id= taskInfo.ActividadId.toString(),
                            description = if (!queryDescription.matches(notEmptyOrSpacesRegex)){taskInfo.Descripcion} else {queryDescription},
                            error = if (!queryError.matches(notEmptyOrSpacesRegex)){taskInfo.MargenError.toString()} else {queryError},
                            area = if (!queryArea.matches(notEmptyOrSpacesRegex)){taskInfo.Area} else {queryArea},
                            numUser = if (!queryNumUser.matches(notEmptyOrSpacesRegex)){taskInfo.NumEmpleado.toString()} else {queryNumUser},
                            estado = "En proceso",
                            dateInit = if (!selectedDateInit.matches(notEmptyOrSpacesRegex)){taskInfo.FechaInicio} else {selectedDateInit},
                            dateFinish = if (!selectedDateFinish.matches(notEmptyOrSpacesRegex)){taskInfo.FechaFin} else {selectedDateFinish},
                        )
                        if (textAlert == "Good") {
                            showDialog = true
                        }
